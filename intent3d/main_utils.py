...
def parse_option():
    ...
    parser.add_argument('--tensorboard',
                        default='/data/kangweitai/3D/intention/tensorboard/tmp/',
                        help='dir path to save tensorboard')

    args, _ = parser.parse_known_args()

    args.eval = args.eval_val or args.eval_test
...
def load_checkpoint(args, model, optimizer, scheduler):
    ...
    if not args.eval and not args.reduce_lr:
        ...
        print(f'--- suceed to load optimizer and scheduler ---')

    print("=> loaded successfully '{}' (epoch {})".format(
        args.checkpoint_path, checkpoint['epoch']
    ))

    best_ap_25 = checkpoint['best_ap_25']
    best_ap_50 = checkpoint['best_ap_50']

    del checkpoint
    torch.cuda.empty_cache()
    return best_ap_25, best_ap_50

# save model.
def save_checkpoint(args, epoch, model, optimizer, scheduler, best_ap_25=0, best_ap_50=0, name='best'):
    ...
    state = {
        ...
        'epoch': epoch,
        'best_ap_25': best_ap_25,
        'best_ap_50': best_ap_50
    }
    
    spath = os.path.join(args.log_dir, f'ckpt_{name}.pth')
...
class BaseTrainTester:
    ...
    def __init__(self, args):
        ...
        if not (args.eval_val or args.eval_test):
            self.tensorboard = record_tensorboard.TensorBoard(args.tensorboard, distributed_rank=dist.get_rank())
    ...
    def main(self, args):
        ...
        if args.checkpoint_path:
            ...
            _, best_ap_50 = load_checkpoint(args, model, optimizer, scheduler)
        ...
        for epoch in range(args.start_epoch, args.max_epoch + 1):
            ...
            self.logger.info(
                'epoch {}, total time hours{:02d} minutes:{:02d} seconds:{:05.2f}, '
            ...
            if epoch % args.val_freq == 0:
                ...
                minutes = int((elapsed_time % 3600) // 60)
                seconds = elapsed_time % 60
                print(f'Validation time hours:{hours:02d} minutes:{minutes:02d} seconds:{seconds:05.2f}')

                if dist.get_rank() == 0:
                    ap_25 = APs[0]
                    ap_50 = APs[1]
                    save_checkpoint(args, epoch, model, optimizer, scheduler, 
                                    best_ap_25=ap_25, best_ap_50=ap_50, name='last')
                    
                    if ap_50 >= best_ap_50:
                        best_ap_50 = ap_50
                        save_checkpoint(args, epoch, model, optimizer, scheduler, 
                                        best_ap_25=ap_25, best_ap_50=ap_50, name='best')
