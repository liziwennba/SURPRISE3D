...
class BeaUTyDETR(nn.Module):
    def __init__(self, num_class=256, num_obj_class=485,
        ...
        # Text Encoder
        self.tokenizer = RobertaTokenizerFast.from_pretrained("/workspace/huggingface/roberta-base/")
        self.text_encoder = RobertaModel.from_pretrained("/workspace/huggingface/roberta-base/")
        ...
        self.pos_embed_pg = PositionEmbeddingLearned(6, d_model)
    ...
    def _run_backbones(self, inputs):
        ...
        end_points['seed_labels'] = torch.gather(inputs['point_instance_label'], 1, 
    ...
    def forward(self, inputs):
        ...
        pos_embed_pg = self.pos_embed_pg(inputs['det_boxes'])
        
        # STEP 3. Cross-modality encoding
        points_features, text_feats, detected_feats = self.cross_encoder(
            ...
            detected_feats_pos = pos_embed_pg.transpose(1, 2).contiguous()
        )
        ...
        if self.contrastive_align_loss:
            ...
            end_points['v_text'] = self.v_text(text_feats)  # ([B, L, 64])
            end_points['o_text'] = self.o_text(text_feats)
        ...
        if self.contrastive_align_loss: 
            ...
            end_points['proposal_vo_query']=self.vo_query(query)
        ...
        for i in range(self.num_decoder_layers):
            ...
            if self.contrastive_align_loss:
                ...
                end_points[f'{prefix}vo_query'] = self.vo_query(query)
